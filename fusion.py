...
def convert_weights(train_weights, deploy_weights, eps):
    train_dict = torch.load(train_weights)['state_dict'] # train_weights是扩展后网络训练的结果，deploy_weights是合并之后
    
    #print(train_dict.keys())
    deploy_dict = {}
    square_conv_var_names = [name for name in train_dict if SQUARE_KERNEL_KEYWORD in name]
    square_conv_var_names = [name for name in train_dict.keys() if SQUARE_KERNEL_KEYWORD in name] #原始网络的卷积keys
...
if __name__ == '__main__':
    ...
    target_weights = '/home/zhangxin/new/weights/checkpoint_500.pth' 
