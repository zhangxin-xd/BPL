...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes_after_prune, planes_expand, planes_before_prune, index, bn_value, index_conv2,bias_value,stride=1,
                 downsample=None):
        ...
        self.conv3 = nn.Conv2d(planes_before_prune, planes_expand, kernel_size=1, bias=False)
        ...
        self.index_conv2 = Variable(index_conv2)
    ...
    def forward(self, x):
        ...
        out = self.conv2(out) # 39
        out_with_bias = self.bias_value.cuda() # 64
        out_with_bias_expand = out_with_bias.view(1,out_with_bias.shape[0],1,1) # 1 64 1 1
        a = torch.ones(out.shape[0], out_with_bias.shape[0], out.shape[2], out.shape[3]).cuda()
        b =  a*out_with_bias_expand
        b.index_add_(1, self.index_conv2.cuda(), out)
        
        
        # out = self.bn2(out)
        out = self.relu(b)

        out = self.conv3(out) # 154
        out = self.bn3(out)

        if self.downsample is not None:
            residual = self.downsample(x) # 256
        ...
        # setting: with index match
        residual += self.bn_value.cuda()  # 256?????
        residual.index_add_(1, self.index.cuda(), out) # self.index是conv3保留的filters的index
...
class ResNet_small(nn.Module):
    ...
    def __init__(self, block, layers, index, bn_value, bias_value,
        ...
        self.bias_layer1 = {key: bias_value[key] for key in bias_value.keys() if 'layer1' in key}
        self.bias_layer2 = {key: bias_value[key] for key in bias_value.keys() if 'layer2' in key}
        self.bias_layer3 = {key: bias_value[key] for key in bias_value.keys() if 'layer3' in key}
        self.bias_layer4 = {key: bias_value[key] for key in bias_value.keys() if 'layer4' in key}

        # print("bn_layer1", bn_layer1.keys(), bn_layer2.keys(), bn_layer3.keys(), bn_layer4.keys())

        self.layer1 = self._make_layer(block, num_for_construct[1], num_for_construct[2], 64, self.index_layer1, self.bn_layer1, self.bias_layer1,
                                       layers[0])
        self.layer2 = self._make_layer(block, num_for_construct[3], num_for_construct[4], 128, self.index_layer2, self.bn_layer2, self.bias_layer2,
                                       layers[1], stride=2)
        self.layer3 = self._make_layer(block, num_for_construct[5], num_for_construct[6], 256, self.index_layer3, self.bn_layer3, self.bias_layer3,
                                       layers[2], stride=2)
        self.layer4 = self._make_layer(block, num_for_construct[7], num_for_construct[8], 512, self.index_layer4, self.bn_layer4, self.bias_layer4,
                                       layers[3], stride=2)
    ...
    def _make_layer(self, block, planes_after_prune, planes_expand, planes_before_prune, index, bn_layer, bias_layer, blocks,
                    stride=1):
        downsample = None
        ...
        index_block_0_conv2_dict = {key: index[key] for key in index.keys() if '0.conv2' in key}
        index_block_0_conv2_value = list(index_block_0_conv2_dict.values())[0]
        bn_layer_0_value = list(bn_layer.values())[0]
        bias_layer_0_value = list(bias_layer.values())[0]

        layers = []
        layers.append(
            block(self.inplanes, planes_after_prune, planes_expand, planes_before_prune, index_block_0_value,
                  bn_layer_0_value, index_block_0_conv2_value, bias_layer_0_value,
                  stride, downsample))
        ...
        for i in range(1, blocks):
            ...
            index_block_i_conv2_dict = {key: index[key] for key in index.keys() if (str(i) + '.conv2') in key}
            index_block_i_conv2_value = list(index_block_i_conv2_dict.values())[0]
            bias_layer_i = {key: bias_layer[key] for key in bias_layer.keys() if (str(i) + '.conv2.bias') in key}
            bias_layer_i_value = list(bias_layer_i.values())[0]
            layers.append(
                block(self.inplanes, planes_after_prune, planes_expand, planes_before_prune, index_block_i_value,
                      bn_layer_i_value, index_block_i_conv2_value, bias_layer_i_value
                      ))
        return nn.Sequential(*layers)

    def forward(self, x):
        x = self.conv1(x) # 3 39 128 128  / 112
        x = self.bn1(x)
        x = self.relu(x)
        x = self.maxpool(x) # 64 64 64 / 56

        x = self.layer1(x) # 256 64 64  /56
        x = self.layer2(x)  # 512 32 32   / 28
        x = self.layer3(x)   # 1024 16 16   / 14
        x = self.layer4(x)  # 2048 8 8    /7

        x = self.avgpool(x) # 2048 2 2  / 1
